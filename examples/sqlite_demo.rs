...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Save the changes
    pm_sqlite.save_now().await.expect("Failed to save device data");
    ...
    pm_file.modify_device(|device| {
        device.push_name = "FileStore Demo Device".to_string();
    }).await;
    
    pm_file.save_now().await.expect("Failed to save device data");
    ...
    println!("Persisted device push_name from previous session: '{}'", persisted_device.push_name);
