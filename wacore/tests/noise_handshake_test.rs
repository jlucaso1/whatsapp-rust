...
fn test_server_static_key_decryption_with_go_values() {
    ...
    let server_eph_pub =
        hex::decode("9d9b8241572937f6672ce07221c143bc1cb1266334333f33797a592669523733").unwrap();
    let salt_before_mix =
        hex::decode("4e6f6973655f58585f32353531395f41455347434d5f53484132353600000000").unwrap();
    let go_salt_after_mix =
        hex::decode("cf20e4a2d076541a3f45609407f6501d9f20d55f40b06ff32af4573c44ab1595").unwrap();
    let aad_for_decrypt =
        hex::decode("42e2950db11903f6fd490bedd1cc630feb40b1d01ac350655a98273284498a1d").unwrap();
    let ciphertext = hex::decode("6de6577bb273d20bb64ee96ffc99fabd1390853791c26fad729806b5b7ddedc44d689b48e05068e2823b44dd533fff8d").unwrap();
...
fn test_full_handshake_flow_with_go_data() {
    ...
    let aad_for_decrypt_1 =
        hex_to_bytes::<32>("9fd7bf2736e1e5b48b2759fd5806225ccb6822d557579d725459795cdd0885d3");
    let ciphertext_for_decrypt_1 = hex::decode("0addd1ce96c55699824fc20ef2a73928dd8168bc29ef863bdea6dc69002b00054add28f7711e3a7dff984d3f31badaa2").unwrap();
    ...
    let decrypted_static_bytes = nh.decrypt(&ciphertext_for_decrypt_1).unwrap();
    assert_eq!(
    ...
    let decrypted_cert_bytes = nh.decrypt(&ciphertext_for_decrypt_2).unwrap();
    assert_eq!(
