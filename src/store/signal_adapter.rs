...
use wacore::signal::ecc::keys::EcPrivateKey as WacoreEcPrivateKey;
use wacore::signal::store::{
    IdentityKeyStore as WacoreIdentityKeyStore, PreKeyStore as WacorePreKeyStore,
...
// --- Create separate adapter structs for each store trait ---
...
// --- Implement the traits on the new, smaller structs ---
...
impl IdentityKeyStore for IdentityAdapter {
    async fn get_identity_key_pair(&self) -> Result<IdentityKeyPair, SignalProtocolError> {
        let device = self.0.device.lock().await;
        let wacore_kp = WacoreIdentityKeyStore::get_identity_key_pair(&*device)
            .await
            .map_err(|e| {
                SignalProtocolError::InvalidState("get_identity_key_pair", e.to_string())
            })?;
        let public_key =
            PublicKey::from_djb_public_key_bytes(&wacore_kp.public_key().public_key().public_key)?;
        let private_key =
            PrivateKey::deserialize(&wacore_kp.private_key().private_key.serialize())?;
    ...
    async fn get_local_registration_id(&self) -> Result<u32, SignalProtocolError> {
        let device = self.0.device.lock().await;
        WacoreIdentityKeyStore::get_local_registration_id(&*device)
            .await
            .map_err(|e| {
    ...
    async fn save_identity(
        ...
        let wacore_address = ProtocolAddress::new(address.name().to_string(), address.device_id());
        let wacore_identity =
            wacore::signal::identity::IdentityKey::deserialize(&identity.serialize())
                .map_err(|e| SignalProtocolError::InvalidState("deserialize", e.to_string()))?;
        WacoreIdentityKeyStore::save_identity(&*device, &wacore_address, &wacore_identity)
            .await
    ...
    async fn is_trusted_identity(
        ...
        let wacore_identity =
            wacore::signal::identity::IdentityKey::deserialize(&identity.serialize())
                .map_err(|e| SignalProtocolError::InvalidState("deserialize", e.to_string()))?;
        WacoreIdentityKeyStore::is_trusted_identity(&*device, &wacore_address, &wacore_identity)
            .await
...
// Helper functions to convert between wacore and libsignal types
