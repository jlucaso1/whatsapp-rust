...
impl StoreManager {
    ...
    pub async fn get_persistence_manager(&self, device_id: i32) -> Result<Arc<PersistenceManager>, StoreError> {
...
mod tests {
    ...
    async fn create_test_store_manager() -> StoreManager {
        let temp_db = format!("file:memdb_{}?mode=memory&cache=shared", uuid::Uuid::new_v4());
        StoreManager::new(&temp_db).await.expect("Failed to create test StoreManager")
    }
    ...
    async fn test_create_new_device() {
        ...
        // Create a new device
        let manager = store_manager.create_new_device().await.expect("Failed to create new device");
    ...
    async fn test_get_persistence_manager_cached() {
        ...
        // Create a new device
        let manager1 = store_manager.create_new_device().await.expect("Failed to create new device");
        let device_id = manager1.device_id();
        
        // Get it again - should return the same cached instance
        let manager2 = store_manager.get_persistence_manager(device_id).await.expect("Failed to get manager");
    ...
    async fn test_list_devices() {
        ...
        let devices = store_manager.list_devices().await.expect("Failed to list devices");
        assert!(devices.is_empty());
        
        // Create a device
        let _manager1 = store_manager.create_new_device().await.expect("Failed to create device");
        let devices = store_manager.list_devices().await.expect("Failed to list devices");
        assert_eq!(devices.len(), 1);
        
        // Create another device
        let _manager2 = store_manager.create_new_device().await.expect("Failed to create device");
        let devices = store_manager.list_devices().await.expect("Failed to list devices");
    ...
    async fn test_delete_device() {
        ...
        // Create a device
        let manager = store_manager.create_new_device().await.expect("Failed to create device");
        let device_id = manager.device_id();
        
        // Verify it exists
        assert!(store_manager.device_exists(device_id).await.expect("Failed to check device existence"));
        
        // Delete it
        store_manager.delete_device(device_id).await.expect("Failed to delete device");
