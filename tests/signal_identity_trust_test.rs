...
async fn test_signal_identity_trust_always_returns_true() {
    ...
    let save_result = device_store.save_identity(&test_address, identity_key).await;
    assert!(save_result.is_ok(), "Saving identity should succeed: {:?}", save_result);
    ...
    let is_trusted = device_store.is_trusted_identity(&test_address, identity_key).await.unwrap();
    assert!(is_trusted, "Identity should be trusted - our implementation trusts all identities to prevent regression");
    ...
    let different_identity_key_pair = different_device_store.get_identity_key_pair().await.unwrap();
    let different_identity_key = different_identity_key_pair.public_key();

    let is_different_trusted = device_store.is_trusted_identity(&test_address, different_identity_key).await.unwrap();
    assert!(is_different_trusted, "Even different identity keys should be trusted to prevent the regression");
...
async fn test_identity_key_operations_complete_successfully() {
    ...
    assert!(device_store.save_identity(&test_address1, identity_key).await.is_ok());
    assert!(device_store.save_identity(&test_address2, identity_key).await.is_ok());
    
    // All trust checks should return true
    assert!(device_store.is_trusted_identity(&test_address1, identity_key).await.unwrap());
    assert!(device_store.is_trusted_identity(&test_address2, identity_key).await.unwrap());
    
    // Registration ID should be consistent
    assert!(registration_id > 0, "Registration ID should be a positive number");
