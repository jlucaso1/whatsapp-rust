...
async fn test_all_decryption_bundles() {
    ...
    if !bundles_dir.exists() {
        println!(
            "⚠️  No decryption bundles directory found at {}",
            bundles_dir.display()
        );
        println!(
            "   Create {} and add test bundles to enable automated validation",
            bundles_dir.display()
        );
        return;
    }

    let mut entries = fs::read_dir(bundles_dir)
        .await
        .expect("Failed to read bundles directory");
        ...
        .expect("Failed to read directory entry")
    {
        ...
        if path.is_dir() {
            ...
            if output.status.success() {
                println!("✅ Bundle {bundle_name} validated successfully");
                success_count += 1;
            } else {
                println!("❌ Bundle {bundle_name} validation failed");
    ...
    if bundle_count == 0 {
        ...
        println!("🎉 Validated {success_count}/{bundle_count} bundles successfully");
    }
}
...
async fn test_bundle_structure_validation() {
    ...
    assert!(
        stderr.contains("Missing message.bin"),
        "Should report missing message.bin"
    );
}
