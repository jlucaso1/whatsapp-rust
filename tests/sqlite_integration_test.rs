...
async fn test_sqlite_store_basic_operations() {
    ...
    // Put identity
    store.put_identity(address, key).await.expect("Failed to put identity");
    
    // Load identity
    let loaded_identity = store.load_identity(address).await.expect("Failed to load identity");
    ...
    store.put_session(address, &session_data).await.expect("Failed to put session");
    
    let loaded_session = store.get_session(address).await.expect("Failed to get session");
    assert!(loaded_session.is_some());
    assert_eq!(loaded_session.unwrap(), session_data);
    
    // Test has_session
    let has_session = store.has_session(address).await.expect("Failed to check has_session");
    assert!(has_session);
    
    // Test sender key operations
    let sender_key_data = vec![6, 7, 8, 9, 10];
    store.put_sender_key(address, &sender_key_data).await.expect("Failed to put sender key");
    
    let loaded_sender_key = store.get_sender_key(address).await.expect("Failed to get sender key");
    ...
    store.set_app_state_sync_key(&key_id, app_state_key.clone()).await.expect("Failed to set app state key");
    
    let loaded_app_state_key = store.get_app_state_sync_key(&key_id).await.expect("Failed to get app state key");
...
async fn test_persistence_manager_with_sqlite() {
    ...
    let pm = Arc::new(PersistenceManager::new_sqlite(db_path).await.expect("Failed to create PersistenceManager with SQLite"));
    ...
    let pm2 = Arc::new(PersistenceManager::new_sqlite(db_path).await.expect("Failed to create second PersistenceManager"));
...
async fn test_sqlite_store_vs_memory_store() {
    ...
    pm_sqlite.modify_device(|device| {
        device.push_name = test_push_name.clone();
    }).await;
    
    pm_memory.modify_device(|device| {
        device.push_name = test_push_name.clone();
    }).await;
