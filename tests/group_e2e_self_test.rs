...
struct TestEventHandler {
    sender: mpsc::UnboundedSender<(Box<waproto::whatsapp::Message>, wacore::types::message::MessageInfo)>,
}

impl EventHandler for TestEventHandler {
    fn handle_event(&self, event: &Event) {
        println!("TestEventHandler received event: {:?}", std::mem::discriminant(event));
        if let Event::Message(msg, info) = event {
            println!("Got message event! Sender: {}, Text: {:?}", info.source.sender, msg.conversation);
...
) -> (Arc<Client>, mpsc::UnboundedReceiver<(Box<waproto::whatsapp::Message>, wacore::types::message::MessageInfo)>) {
...
async fn test_group_self_communication() {
    ...
    client2
        .group_cache
        .insert(group_jid.clone(), group_info);
    ...
    client2.core.event_bus.dispatch(&wacore::types::events::Event::Message(mock_message, mock_info));
    ...
    println!("Client 2 received message: '{conversation_text}' from sender: {}", info.source.sender);
