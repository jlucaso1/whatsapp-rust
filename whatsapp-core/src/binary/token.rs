...
// NOTE: You must populate these arrays with all tokens from the Go file.
...
static SINGLE_BYTE_MAP: Lazy<HashMap<&'static str, u8>> = Lazy::new(|| {
...
static DOUBLE_BYTE_MAP: Lazy<HashMap<&'static str, (u8, u8)>> = Lazy::new(|| {
    let mut map = HashMap::new();
    for (dict_idx, &dict) in DOUBLE_BYTE_TOKENS.iter().enumerate() {
        for (token_idx, &token) in dict.iter().enumerate() {
            map.insert(token, (dict_idx as u8, token_idx as u8));
        }
    }
    map
});
